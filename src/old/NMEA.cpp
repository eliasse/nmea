...
void NMEA::decode_VTG(char * tok, uint8_t n_fields, bool is_empty[])
{
...
void NMEA::decode_GGA(char * tok, uint8_t n_fields, bool is_empty[])
{
  for (uint8_t element = 2; element <= n_fields; element++)
    {
      if (is_empty[element] == true) { continue; }
...
void NMEA::decode_GLL(char * tok, uint8_t n_fields, bool is_empty[])
{
  for (uint8_t element = 2; element <= n_fields; element++)
    {
      if (is_empty[element] == true) { continue; }
	...
	case GLL_ELEMENTS::VALIDITY:
	  ...
	  gll->valid        = atoi(gll_str->validity);
	  break;
	}
    }
}
...
void NMEA::decode_RMC(char * tok, uint8_t n_fields, bool is_empty[])
{
  ...
  for (uint8_t element = 2; element <= n_fields; element++) {
	...
	break;
      ...
      case RCM_ELEMENTS::TC: // True Course
