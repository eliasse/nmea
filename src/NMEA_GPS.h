...
namespace RMC_ELEMENTS { // Defines the field locations in an NMEA sentence
    const uint8_t UTC = 2;
...
class NMEA_GPS {
 public:
  ...
  struct RMC {  // Decoded RMC Contents //
    char rmc[100]; // Contains a copy of the RMC sentence
    // UTC Field
    int utc;
    uint8_t second;
    uint8_t minute;
  ...
  char nmea_copy[100]; // Copy for debug
